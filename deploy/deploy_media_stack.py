...
def main() -> None:
    ...
    media_archive = Path(__file__).with_name(f"mediamtx_v{MEDIA_VERSION}_linux_amd64.tar.gz")
    ...
    cloud = connect(CLOUD_HOST, "root", cloud_password)
    pi = None
    try:
        upload(cloud, DASHBOARD_DEPLOY / "mediamtx.yml.template", "/tmp/visionbridge-mediamtx.yml.template")
        upload(cloud, DASHBOARD_DEPLOY / "turnserver.conf.template", "/tmp/visionbridge-turnserver.conf.template")
        upload(cloud, DASHBOARD_DEPLOY / "visionbridge-mediamtx.service", "/tmp/visionbridge-mediamtx.service")
...
""")
        ...
        publisher_env = (
            "VISIONBRIDGE_DEVICE_ID=uno-cloud-gateway-01\n"
...
""")
        pi_result = run(pi, r"""set -Eeuo pipefail
touch /home/pi/.ssh/known_hosts
chmod 600 /home/pi/.ssh/known_hosts
grep -v '^115\.231\.176\.136 ' /home/pi/.ssh/known_hosts > /tmp/known_hosts.preserved || true
...
""", timeout=180)
        ...
        final = run(cloud, r"""set -Eeuo pipefail
for i in $(seq 1 30); do
  READY=$(curl -fsS http://127.0.0.1:9997/v3/paths/list | python3 -c 'import json,sys; d=json.load(sys.stdin); print(any(x.get("name") == "devices/uno-cloud-gateway-01" and x.get("ready") for x in d.get("items", [])))')
  [ "$READY" = True ] && break
  sleep 1
done
[ "$READY" = True ]
curl -fsS http://127.0.0.1:8889/devices/uno-cloud-gateway-01/ >/dev/null
curl -fsS http://127.0.0.1:8888/devices/uno-cloud-gateway-01/ >/dev/null
