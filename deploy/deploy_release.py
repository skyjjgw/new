...
python3 -m py_compile "$RELEASE/web-dashboard/server/app.py"
test -s "$RELEASE/web-dashboard/static-deploy/index.html"
test -s "$RELEASE/volunteer-app/build/web/main.dart.js"
grep -q 'Temporary competition/demo mode' "$RELEASE/web-dashboard/deploy/nginx-visionbridge.conf"
...
cp -a "$RELEASE/web-dashboard/server" "/opt/visionbridge/server.next-$STAMP"
cp -a "$RELEASE/web-dashboard/static-deploy" "/opt/visionbridge/static-deploy.next-$STAMP"
cp -a "$RELEASE/volunteer-app/build/web" "/opt/visionbridge/volunteer.next-$STAMP"
...
cp -a "$RELEASE/web-dashboard/deploy/nginx-visionbridge.conf" "$NGINX"
SWAPPED=1
